...
export const careerDetails: CareerDetail[] = [
  // ── 뭉클랩 ──────────────────────────────────────────────────────
  {
    ...
    companyId: 'moongklab',
    companyName: '(주)뭉클랩',
    projectName: '장사왕 — 쿠팡 광고 분석 고도화 및 AI 진단 서비스',
    summary:
      '쿠팡·네이버스마트스토어 셀러 대상 광고 분석 SaaS에서, 숫자 나열 대시보드 → 룰베이스 분석 → AI 에이전트 기반 진단으로 3단계 고도화하여 AI 광고 진단 서비스 출시',
    problem:
      '셀러가 광고 성과를 숫자로만 확인할 수 있어, 무엇을 개선해야 하는지 스스로 판단하기 어려운 구조. 광고비는 쓰지만 효율이 나빠지는 이유를 모르고, 대시보드의 수치는 해석 없이 나열되어 액션으로 이어지지 않았음.',
    hypothesis:
      '광고 데이터를 단순 시각화하는 것을 넘어, 룰베이스 조건 분석과 AI 에이전트 기반 진단을 단계적으로 도입하면 — 셀러가 "무엇을 고쳐야 하는지" 바로 알 수 있는 액셔너블한 인사이트를 제공할 수 있을 것이다.',
    actions: [
      {
        title: '쿠팡 광고 분석 기능 출시 및 단계적 고도화',
        details: [
          '1단계: 쿠팡 광고 데이터 수집·시각화 대시보드 기획 — ROAS, CPC, 전환율 등 핵심 지표 설계',
          '2단계: 룰베이스 분석 도입 — 조건 기반 자동 진단으로 "ROAS 낮은 키워드", "클릭 대비 전환 부진 광고" 등 패턴 자동 탐지',
          '3단계: AI 에이전트 기반 진단 서비스 — LLM이 셀러별 광고 데이터를 종합 분석하여 맞춤형 개선 액션 제안',
        ],
      },
      {
        title: 'AI 진단 서비스 설계 및 출시',
        details: [
          '룰베이스 진단과 LLM 기반 진단의 비교 실험을 설계·실행하여 최적 접근법 도출',
          '실험 결과를 바탕으로 AI 에이전트 기반 광고 진단 서비스 스펙 수립 및 프로덕션 출시',
        ],
      },
      {
        title: '기획 프로세스 체계화',
        details: [
          ...
          'AI 멀티에이전트 시스템을 활용한 기획 워크플로우 운영 (→ 개인 오픈소스 Popilot으로 발전)',
        ],
      },
    ],
    results: [
      '쿠팡 광고 분석 기능 출시 — 숫자 나열 → 룰베이스 → AI 에이전트 3단계 고도화 완료',
      '셀러 대상 AI 광고 진단 서비스 프로덕션 출시',
      '스프린트 프로세스 체계 도입으로 기획-개발 사이클 정립',
      'AI 멀티에이전트 기획 경험을 개인 오픈소스 프로젝트 Popilot으로 추출',
    ],
    insights: [
      '데이터 시각화 → 분석 → 진단으로 진화할수록 사용자가 "다음에 뭘 해야 하는지"에 가까워짐. 대시보드의 가치는 숫자가 아니라 액션',
      'AI를 "도구로 쓰는 것"과 "워크플로우 자체를 AI 네이티브로 설계하는 것"은 본질적으로 다른 접근. 기획 과정에서 멀티에이전트를 실전 적용하며 그 차이를 체득',
    ],
  },
  ...
  // ── 헤바 ────────────────────────────────────────────────────────
  {
    ...
    summary:
      'Slack, GitHub, Notion, Jira에 흩어진 팀 의사결정을 검색 가능한 자산으로 전환하는 AI decision memory product',
    problem:
    ...
    actions: [
      ...
      {
        title: '에이전트 기반 개발 파이프라인 설계',
        details: [
          '계획→구현→검증→리뷰 5단계 게이트 시스템 설계',
          '역할 분리와 크로스모델 검증 원칙을 적용해 품질 확보 구조 설계',
          '실제 이슈·PR 단위로 반복 운영 가능한 개발 루프 구축',
        ],
      },
      {
        title: '인프라 구축',
    ...
    results: [
      'MCP 서버를 통해 AI 에이전트가 팀 의사결정을 검색 가능한 프로덕트 출시 (corti.wiki)',
      '커넥터 4종(Slack, GitHub, Notion, Jira) 연동 완료',
      '에이전트 기반 개발 파이프라인을 실제 제품 개발에 운영하며 PR 단위 협업 자동화 경험 축적',
      'API Key 관리, 하이브리드 검색, Context Tree 등 핵심 기능 지속 확장',
    ],
    insights: [
      'AI 에이전트는 "코드를 대신 짜는 도구"보다 "서로 다른 관점으로 검증하는 팀원"으로 설계할 때 품질이 올라감을 확인',
  ...
  },
  {
    ...
    companyId: 'heavaa',
    companyName: '헤바 (Heavaa)',
    projectName: 'Sprintable — AI Native 팀 운영 워크플로우 제품',
    summary:
      'AI 에이전트와 사람이 함께 일하는 스프린트 운영체계를 제품화한 AI Native workflow product',
    problem:
      '기존 PM 도구는 사람 중심으로 설계되어 있어, AI 에이전트가 backlog·문서·메모·스프린트 라이프사이클을 네이티브하게 운영하기 어려웠음. 에이전트를 코딩 도구로만 쓰는 수준을 넘어, 팀 운영 체계 자체를 AI와 함께 작동하게 만드는 제품이 필요했음.',
    hypothesis:
      'PM 워크플로우 자체를 에이전트 친화적인 구조로 재설계하고, 메모·넛지·문서·칸반·MCP 도구를 하나의 운영 체계로 연결하면, HITL을 최소화하면서도 재현 가능한 협업 플로우를 만들 수 있을 것이다.',
    actions: [
      {
        title: '문제 재정의 및 제품 구조 설계',
        details: [
          'sellerking-sprint-planner에서 검증한 운영 패턴을 추출해 별도 제품 방향으로 재구성',
          '"방법론 보급"이 아니라 "팀 운영 체계(OS)"를 설계하는 관점으로 문제 정의 전환',
          '오픈소스 코어 + 에이전트 서빙 레이어로 이어지는 3-Layer 제품 구조 설계',
        ],
      },
      {
        title: 'PM 코어 워크플로우 제품화',
        details: [
          '메모, 스프린트, 회고, 문서, 회의록, 칸반/로드맵을 하나의 운영 체계로 연결',
          'Next.js, Supabase, GCP 중심의 리빌드 청사진과 정보 구조 설계',
          '에이전트가 접근할 수 있는 MCP 도구 인터페이스와 팀용 UI를 함께 설계',
        ],
      },
      {
        title: 'Dogfooding 및 공동사업 전환',
        details: [
          '독립된 PO/Dev/QA 에이전트가 이 도구 자체를 개발하는 dogfooding 구조 운영',
          '메모 기반 비동기 지휘체계와 검증 게이트를 실제 협업 플로우에 적용',
          '뭉클랩과의 공동사업 구조로 제품화 방향 정리',
        ],
      },
    ],
    results: [
    ...
    insights: [
      '팀이 실패하는 이유는 방법론 부재보다 팀의 실제 운영 체계와 도구가 맞지 않기 때문이라는 점을 확인',
      '에이전트 시대의 제품은 "AI가 뭘 대신 해주나"보다 "사람과 에이전트가 어떤 구조로 함께 일하나"를 설계해야 함',
    ],
  },
  {
    id: 'maldives-match',
  ...
  },
  {
    ...
    results: [
      'GA, Amplitude, Airbridge 등으로 분산된 KPI 정의를 통합하는 데이터 기준 마련',
      'Kafka ETL, 자체 대시보드, Airflow 자동 리포팅으로 의사결정용 데이터 접근성 개선',
      '경영진과 실무팀이 동일한 기준으로 데이터를 해석할 수 있는 운영 기반 구축',
    ],
    insights: [
      'BA의 역할은 리포트 작성자가 아니라 의사결정의 촉매자에 가깝다는 점을 학습',
      '데이터의 해석보다 먼저 데이터의 일관성을 설계해야 한다는 점을 체득',
    ],
  },
