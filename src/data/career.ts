...
export const careerDetails: CareerDetail[] = [
  // ── 뭉클랩 ──────────────────────────────────────────────────────
  {
    ...
    actions: [
      {
        ...
        details: [
          ...
          '3개 스프린트(S52~S54)에 걸쳐 실전 운영 — 168 커밋, 568개 파일, 175개 스토리 문서 생산',
        ],
      },
      {
        ...
        details: [
          ...
          'spec.moonklabs.com 배포 — 팀 전체가 실시간으로 스펙을 확인하고 메모/리뷰하는 협업 환경 구축',
        ],
      },
      {
    ...
    results: [
      '3개 스프린트 실전 운영 — AI 에이전트 협업으로 175개 스토리 문서, 12개 에픽 스펙 생산',
      'spec-site 구축 및 배포 — PO가 직접 인터랙티브 목업을 만들어 개발 핸드오프하는 새로운 워크플로우 확립',
      'AI 진단 V2 스펙 도출 및 프로덕션 배포 — 47개 진단 규칙 + MarginRoom 설정',
      'S52 대실패(KR 5개 중 1개 달성)에서 "AI가 빠르게 만드는 건 문서이지 의사결정이 아니다"라는 핵심 교훈 도출 → Corti 탄생의 계기',
    ...
    actions: [
      {
        title: '실전 경험에서 코어 추출',
        details: [
          '뭉클랩 568개 파일, 175개 스토리, 3스프린트 운영 경험에서 핵심 패턴 추출',
          '과잉 설계(15명 에이전트)를 정리하여 실제 사용 빈도 기반 핵심 에이전트로 경량화',
          'S52 대실패(KR 5개 중 1개 달성) 교훈을 프레임워크 설계 원칙에 반영',
        ],
      },
      {
        title: '에이전트 아키텍처 설계',
